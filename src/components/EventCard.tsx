...
const EventCard = ({ event, index }) => {
  ...
  const GlitchOverlay = ({ isActive }) => {
    return (
      ...
      >
        <div className={`absolute inset-0 ${colorClasses.bg} mix-blend-screen bg-opacity-50`}></div>
      ...
      {event.bannerImage && (
        ...
        >
          ...
          <motion.div 
            ...
            style={{
              background: "linear-gradient(transparent 0%, rgba(0, 0, 0, 0.1) 50%, transparent 100%)",
          ...
          <motion.div 
            ...
            animate={{
              boxShadow: isHovered 
                ? [`inset 0 0 10px ${colorClasses.text}`, `inset 0 0 20px ${colorClasses.text}`, `inset 0 0 10px ${colorClasses.text}`] 
